...
BEGIN
    ...
    -- ============================================================
    -- 7c. Row exists with check_out IS NOT NULL → ALREADY COMPLETE
    --     Strict two-step toggle: reject any further scans for this
    --     business day once both check-in and check-out are logged.
    -- ============================================================
    RETURN jsonb_build_object(
        'success', false,
...
DROP POLICY IF EXISTS salary_payments_select_admin ON public.salary_payments;
CREATE POLICY salary_payments_select_admin
...
DROP POLICY IF EXISTS salary_payments_insert_admin ON public.salary_payments;
CREATE POLICY salary_payments_insert_admin
    ON public.salary_payments
    FOR INSERT
    WITH CHECK (auth.uid() IN (
        SELECT id FROM public.profiles WHERE role = 'admin'
