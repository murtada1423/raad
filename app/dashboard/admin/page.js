      ...
      <div style={{ display: 'flex', flexDirection: 'row', minHeight: '100vh' }}>
        ...
        <div className="adm-side" style={s.sidebar}>
          ...
          <div className="adm-side-nav" style={s.sidebarNav}>
            ...
            <button className={`adm-nav-item${activeTab === 'audit' ? ' adm-nav-active' : ''}`}
              style={{ ...s.navItem, ...(activeTab === 'audit' ? s.navItemActive : {}) }}
        ...
        <div className="adm-main" style={s.mainContent}>
                      ...
                      {employees.map((emp) => (
                        <div key={emp.id} style={{ ...s.tableRow, cursor: 'pointer' }} onClick={() => setSelectedEmployee(emp)}>
                          ...
                          <span style={s.tdAction}>
                            <button style={s.editBtn} onClick={(e) => { e.stopPropagation(); openEdit(emp) }}>تعديل</button>
                            <button style={s.deleteBtn} onClick={(e) => { e.stopPropagation(); setDeleteTarget(emp) }}>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {activeTab === 'audit' && (
            <div style={s.containerInner}>
              <header style={s.header}>
                <div>
                  <h1 style={s.greeting}>سجل التعديلات</h1>
                  <p style={s.role}>جميع التعديلات والإضافات والحذف على سجلات الحضور</p>
                </div>
                <button style={s.refreshBtn} onClick={loadAuditLog}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div style={s.section}>
                {auditLogLoading ? (
                  <div style={s.emptyState}><p>جاري التحميل...</p></div>
                ) : auditLog.length === 0 ? (
                  <div style={s.emptyState}><p>لا توجد سجلات تعديل بعد.</p></div>
                ) : (
                  <div style={s.tableResponsive}>
                    <div style={s.table}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 0.7fr 1fr 1fr 44px', gap: 8, padding: '10px 0', borderBottom: '1px solid rgba(0,0,0,0.06)', marginBottom: 4 }}>
                        <span style={s.th}>التاريخ</span>
                        <span style={s.th}>الموظف</span>
                        <span style={s.th}>الإجراء</span>
                        <span style={s.th}>بواسطة</span>
                        <span style={s.th}>السبب</span>
                        <span style={{ ...s.th, textAlign: 'center' }}></span>
                      </div>
                      {auditLog.map((entry) => {
                        ...
                        return (
                          <div key={entry.id} style={{
                            display: 'grid',
                            gridTemplateColumns: '1.2fr 1fr 0.7fr 1fr 1fr 44px',
                            gap: 8, padding: '10px 0',
                            borderBottom: '1px solid rgba(0,0,0,0.04)',
                            alignItems: 'center',
                          }}>
                            <span style={s.td}><span dir="ltr">{dateStr} {timeStr}</span></span>
                            <span style={{ ...s.tdName, fontSize: 13 }}>{empName}</span>
                            <span>
                              <span style={{
                            ...
                            <span style={s.td}>{profilesMap[entry.changed_by] || 'المدير'}</span>
                            <span style={{ ...s.td, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              {entry.reason || '—'}
                            </span>
                            <span style={{ textAlign: 'center' }}>
                              {entry.record_id && (
                                <button style={s.auditBtn} onClick={() => { setAuditModalRecord(entry.record_id); setActiveTab('audit') }} title="عرض التفاصيل">
